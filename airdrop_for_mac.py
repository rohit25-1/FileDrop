...
def receive():#this function receives the  file
    SEPARATOR="<SEPARATOR>"
    s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    ...
    name="/Users/<NAME OF THE USER ON YOUR PC>/Downloads/"+filename#enter the user name of your pc
    f=open(name,"wb")
    while True:
        datas=s.recv(1024)
        while datas:
...
open_button=tk.Button(labelframe2,text="open",relief=FLAT,width=5,command=open_file)
open_button.grid(row=0,column=1,pady=10)
send_button=tk.Button(labelframe2,text="send",relief=FLAT,width=5,command=send)
send_button.grid(row=0,column=2)
receive_button=tk.Button(labelframe2,text="receive",relief=FLAT,width=5,command=receive)
